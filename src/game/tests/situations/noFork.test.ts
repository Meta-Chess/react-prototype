...
describe("With no fork enabled", () => {
  it("knight cannot fork and pieces cannot allow knight to fork", () => {
    ...
    // Expect allowable moves to be normal
    ...
    expect(gameMaster.allowableMoves).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ location: "R3F5" }),
    ...
    expect(gameMaster.allowableMoves).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ location: "R3F4" }),
        expect.objectContaining({ location: "R4F4" }),
      ])
    );
    expect(gameMaster.allowableMoves.length).toEqual(2);
  ...
  it("it is not check if the king destination would allow the knight to hit 2 enemy pieces", () => {
    ...
    // Expect number of allowable moves to be 2
    expect(gameMaster.allowableMoves.length).toEqual(2);
    ...
    // Expect allowable moves to be normal
    expect(gameMaster.allowableMoves).toEqual(
      expect.arrayContaining([expect.objectContaining({ location: "R6F2" })])
    );
    // Expect number of allowable moves to be 1
    expect(gameMaster.allowableMoves.length).toEqual(1);
    ...
    expect(gameMaster.allowableMoves).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ location: "R7F2" }),
        expect.objectContaining({ location: "R6F1" }),
      ])
    );
    // Expect number of allowable moves to be 2
  ...
  it("fatigue makes it very easy to stalemate", () => {
    ...
    expect(gameMaster.allowableMoves).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ location: "R6F8" }),
        expect.objectContaining({ location: "R5F8" }),
      ])
    );
    expect(gameMaster.allowableMoves.length).toEqual(2);
