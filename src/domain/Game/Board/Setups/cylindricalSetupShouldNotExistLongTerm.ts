...
const square = (rank: number, file: number, pieces: Piece[]): SquareMap => {
  const adjacencies = {
    [Direction.N]: [`R${rank + 1}F${(file - 1).mod(8) + 1}`],
    [Direction.NE]: [`R${rank + 1}F${file.mod(8) + 1}`],
    [Direction.E]: [`R${rank}F${file.mod(8) + 1}`],
    [Direction.SE]: [`R${rank - 1}F${file.mod(8) + 1}`],
    [Direction.S]: [`R${rank - 1}F${(file - 1).mod(8) + 1}`],
    [Direction.SW]: [`R${rank - 1}F${(file - 2).mod(8) + 1}`],
    [Direction.W]: [`R${rank}F${(file - 2).mod(8) + 1}`],
    [Direction.NW]: [`R${rank + 1}F${(file - 2).mod(8) + 1}`],
  };
  return {
